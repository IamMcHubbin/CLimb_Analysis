...
```bash
# Install cloudflared, then authenticate against your domain
...
`docker compose up -d` has a restart policy, and `cloudflared service install`
handles the tunnel, so the machine coming back up is enough.
